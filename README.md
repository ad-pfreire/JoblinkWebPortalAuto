...
| **Subscription** (summary card) | Shows current plan name + trial end date; "Manage Subscription" link goes to the full **`/subscription`** page — Free / Job Link Pro / Job Link Pro + Invoicing plan comparison, each listing feature bullets like "Add New Jobs", "Inspection Checklists", "Photos and Notes", "Customer and Equipment History" (marketing copy on the comparison card, not a real in-app "Jobs" section); covers plan selection, Monthly/Yearly toggle, first purchase via real Stripe Checkout, in-app upgrade/downgrade, cancel/resume, and edge cases. A new "Have a Coupon Code?" field also lives here now — **live and working on staging as of 2026-09-17, still `500`-ing on pre-staging, and automated nowhere**: the plan is written (`specs/company-plans/subscription-coupons-test-plan.md`) and the Stripe key now carries the Coupons/Promotion Codes Write scopes it needs, so this is the one feature a staging release can touch with zero coverage | ✅ (24/25 — 1 known flaky, see below) | `tests/company/subscription.spec.ts` |
